...
def saveHierarchy(hierarchies, search_values):
    ...
    for value_hierarchy, json_hierarchy in hierarchies.items():
        value_hierarchy_obj = HierarchyValue.objects.get_or_create(value=value_hierarchy)
        for (name_hierarchy, array_row) in json_hierarchy.items():
            name_hierarchy_obj = \
                HierarchyName.objects.get_or_create(name=name_hierarchy, hierarchy_value=value_hierarchy_obj[0])
...
def getNamesValues(search_value):
    ...
    data = {}
    hierarchy_search_values = PartUsageHierarchyName.objects.filter(search_value=search_value)
